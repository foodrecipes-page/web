...
/** Pre-generate landing pages for tags with >= 8 recipes. */
export async function generateStaticParams() {
  const all = await loadCatalog();
  return tagFacets(all)
    .filter((f) => f.count >= 8)
    .map((f) => ({ slug: f.slug }));
}
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const all = await loadCatalog();
  const entries = entriesByTag(all, slug);
  if (entries.length === 0) {
  ...
  const description = `Browse ${entries.length} recipes tagged ${label}. Full ingredients, step-by-step instructions, free forever.`;
  return {
    title,
    description,
...
export default async function TagPage({ params }: PageProps) {
  ...
  const breadcrumb = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 3, name: `${label}`, item: `${SITE_URL}/tag/${slug}` },
    ],
  };
  const itemList = {
    "@context": "https://schema.org",
    ...
    <div className="py-4">
      ...
      <div className="text-center mb-8">
        ...
        <p className="mt-2 text-ink-600/80">
          <span className="font-bold text-brand-600">{entries.length}</span> recipes tagged <span className="font-semibold">#{slug}</span>.
        </p>
      </div>
